...
def get_events_geo(events_url, spark):
    ...
    events_geo = spark.read.parquet(raw_path_geo_events)                 .sample(0.1)                 .withColumn('lat2', F.col('lat')/F.lit(57.3))                 .withColumn('lng2', F.col('lon')/F.lit(57.3))                 .withColumn('user_id',
...
def get_events_city(messeges, geo_city):
    ...
    events_city = messages.join(geo_city)                     .withColumn('distance', F.lit(2)*F.lit(6371)*F.asin(F.sqrt(F.pow(F.sin(
                                            ...
                                            + F.cos('lat1') \
                                            * F.cos('lat2') \
                                            * F.pow(F.sin((F.col('lng2')-F.col('lng1'))/F.lit(2)), 2)))) \
...
def get_mart_2(all_events):
    ...
    w = Window
    mart_2 = all_events            .withColumn('week', F.date_trunc('week', F.col('ts')))             .withColumn('month', F.date_trunc('month', F.col('ts')))             .withColumn('action_num', F.rank().over(w().partitionBy('user_id', 'event_type').orderBy('ts')))            .select(
                'month',
                'week',
                'zone_id',
