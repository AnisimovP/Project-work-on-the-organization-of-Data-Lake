...
def get_events_geo(events_url, spark):
    ...
    events_geo = spark.read.parquet(raw_path_geo_events).where(
        ((F.col('event_type') == 'message') & (F.col('event.message_to').isNotNull())) | (
...
def get_mart_3(non_message, user_coordinates):
    mart_3 = non_message.join(user_coordinates, users_pair.user_left == user_coordinates.user_id,
                              'left').withColumnRenamed('user_id', 'lu').withColumnRenamed('act_lat',
                                                                                           'lat1').withColumnRenamed(
        'act_lng', 'lng1').withColumnRenamed('zone_id', 'zone_id1').withColumnRenamed('act_ts', 'act_ts1').join(
        user_coordinates, users_pair.user_right == user_coordinates.user_id, 'left').withColumnRenamed('user_id',
                                                                                                       'ru').withColumnRenamed(
        'act_lat', 'lat2').withColumnRenamed('act_lng', 'lon2').withColumnRenamed('zone_id',
                                                                                  'zone_id2').withColumnRenamed(
        'act_ts', 'act_ts2').withColumn(
        'distance',
